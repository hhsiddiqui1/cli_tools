...
4.  **(Optional) Account Verification:** To mitigate payment failures and fraud, you can implement an account verification step using Monoova's `POST /verify/v1/aba/initiate` API. This sends a small credit with a code to the user's bank account, which they must then provide to you to confirm ownership.
...
4.  **Construct Monoova Payload:** The backend builds the JSON payload for the `POST /financial/v2/transaction/execute` API call.
    *   `callerUniqueReference`: A unique ID generated by you for this payout batch.
    *   `source`: Your primary mAccount number.
    *   `disbursements`: An array containing the payout details (amount, destination bank account or PayID, and a unique reference for the disbursement).
